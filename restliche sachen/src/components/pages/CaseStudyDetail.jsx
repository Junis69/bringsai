            ...
            <div className="hidden sm:flex items-start justify-between relative z-10 w-full mt-2">
                {steps.map((step, idx) => (
                    <div key={idx} className="flex flex-col items-center relative group w-1/5">
                        ...
                        <div className="w-12 h-12 rounded-2xl bg-white border border-[var(--border)] flex items-center justify-center mb-4 relative transition-all duration-300 group-hover:border-[var(--foreground)] group-hover:shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {steps.map((step, idx) => (
                <Reveal key={idx} direction="up" delay={idx * 0.12}>
                    <div className="p-6 rounded-2xl bg-white border border-[var(--border)] relative overflow-hidden group hover:border-[var(--border)] hover:shadow-sm transition-all h-full">
                        <div className="text-[var(--foreground)] font-bold text-sm mb-4 tracking-wider">SCHRITT 0{idx + 1}</div>
    ...
    return (
        <div className="flex flex-wrap gap-3">
            {components.map((comp, idx) => (
                <div key={idx} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-[var(--border)] text-[var(--foreground)]/8 border border-[var(--foreground)]/15">
                    <CheckCircle2 className="w-4 h-4 text-[var(--foreground)]" />
                    <span className="text-sm font-medium text-[var(--foreground)]/80">{comp}</span>
                </div>
            ))}
        </div>
    )
...
function PremiumKPICard({ impactOptions, labels }) {
    return (
        <div className="p-8 rounded-[2rem] bg-white border border-[var(--border)] shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
            <h3 className="text-xs font-bold tracking-[0.2em] text-[var(--foreground)] uppercase mb-8 flex items-center gap-2">
            ...
            <div className="space-y-8 relative z-10">
                {impactOptions.map((impact, i) => (
                    <Reveal key={i} direction="left" delay={i * 0.15} distance={20}>
                        <div className="flex flex-col border-l-2 border-[var(--border)] pl-5 hover:border-[var(--foreground)] transition-colors">
                            <div className="flex items-baseline gap-0 mb-1">
                                {/^[–\-−]/.test(impact.value) ? (
                                    <>
                                        <span className="text-2xl sm:text-3xl lg:text-4xl font-normal tracking-tight text-[var(--foreground)] mr-2">–</span>
                            ...
                            <div className="text-sm font-semibold text-[var(--muted-foreground)]">{impact.label}</div>
                            {impact.context && (
...
function EndCTA({ ctaConfig, language }) {
    ...
    return (
        <Reveal direction="up">
            <div className="mt-32 relative rounded-[2.5rem] bg-[var(--muted)] border border-[var(--border)] overflow-hidden px-6 py-20 text-center hover:border-[var(--border)] transition-all duration-500">
                <div className="relative z-10 max-w-3xl mx-auto flex flex-col items-center">
                    <div className="w-16 h-16 rounded-2xl bg-white border border-[var(--border)] text-[var(--foreground)]/10 flex items-center justify-center mb-8">
                    ...
                    <a
                        ...
                        className="group inline-flex items-center justify-center px-8 py-4 text-sm font-bold bg-white text-[var(--foreground)] border border-[var(--border)] rounded-full transition-all shadow-sm hover:shadow-md hover:border-[var(--foreground)]/20"
                    >
            ...
            <main className="relative pt-32 pb-24 z-10">
                <div className="max-w-6xl mx-auto px-6">
                                        ...
                                        <div className="space-y-6">
                                            {study.solutionBlocks.map((block, i) => (
                                                <Reveal key={i} direction="up" delay={i * 0.12}>
                                                    <div className="flex gap-6 p-6 rounded-2xl bg-white border border-[var(--border)] hover:border-[var(--foreground)]/30 hover:shadow-sm transition-all">
                                                        <div className="w-10 h-10 rounded-full bg-white border border-[var(--border)] text-[var(--foreground)]/10 flex items-center justify-center shrink-0 mt-1">
                                                            <CheckCircle2 className="w-5 h-5 text-[var(--foreground)]" />
                                                        </div>
                                                        <div>
                        ...
                        <motion.div {...fadeUp(0.35)} className="lg:col-span-5 space-y-12">
                            <div className="sticky top-32 space-y-10">
                                ...
                                {study.automatedComponents && study.automatedComponents.length > 0 && (
                                    <div className="p-8 rounded-[2rem] bg-white border border-[var(--border)]">
                    ...
                    <Reveal direction="up" className="mt-32 max-w-4xl mx-auto">
                        ...
                        <blockquote className="relative p-10 md:p-16 rounded-[2.5rem] bg-white border border-[var(--border)] text-center shadow-sm">
                            ...
                            <footer className="relative z-10 flex flex-col items-center justify-center">
                                {study.feedback.image ? (
                                    <img src={study.feedback.image} alt={study.feedback.author} className="w-16 h-16 rounded-full object-cover mb-4 shadow-lg ring-2 ring-[var(--border-light)]" />
                                ) : (
                                    <div className="w-16 h-16 rounded-full bg-white border border-[var(--border)] text-[var(--foreground)] mb-4 shadow-lg flex items-center justify-center">
